...
def test_visibility_error_message_when_user_is_existing(browser, envs):
    login_page = LoginPage(browser)
    registration_page = RegistrationPage(browser)
    login_page.click_create_new_account_button()
    registration_page.send_username(envs['user_name'])
    registration_page.send_password(envs['password'])
    registration_page.send_submit_password(envs['password'])
    registration_page.click_sign_up_button()
    registration_page.check_user_already_exist_on_username_field(envs['user_name'])
