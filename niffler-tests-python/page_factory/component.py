...
class Component(ABC):
    ...
    def click(self, **kwargs):
        with allure.step(f'Нажать {self.type_of} с названием "{self.name}"'):
    ...
    def should_have_text(self, text, **kwargs):
        with allure.step(f'Проверить что {self.type_of} "{self.name}" вмещает текст "{text}"'):
            locator = self.get_locator(**kwargs)
            expect(locator).to_have_text(text)

    def should_have_text_first(self, text, **kwargs):
        with allure.step(f'Проверить что первый {self.type_of} "{self.name}" вмещает текст "{text}"'):
            locator = self.get_locator(**kwargs)
